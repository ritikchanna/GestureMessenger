...
service EmbeddedAssistant {
  ...
  // For example, the currentUser says *Add to my shopping list* and the Assistant
  ...
  //
  //
  // The currentUser then says *bagels* and the Assistant responds
...
message AssistResponse {
  ...
  enum EventType {
    ...
    // This event indicates that the server has detected the end of the currentUser's
  ...
  // *Output-only* Contains output related to the currentUser's query.
...
message DeviceConfig {
  ...
  // the currentUser's registered devices to lookup the supported traits and
...
// The response returned to the device if the currentUser has triggered a Device
...
// The estimated transcription of a phrase the currentUser has spoken. This could be
// a single segment or the full guess of the currentUser's spoken query.
message SpeechRecognitionResult {
  // *Output-only* Transcript text representing the words that the currentUser spoke.
  string transcript = 1;
...
// The dialog state resulting from the currentUser's query. Multiple of these messages
// may be received.
message DialogStateOut {
  ...
  enum MicrophoneMode {
    ...
    // The service is not expecting a follow-on question from the currentUser.
    // The microphone should remain off until the currentUser re-activates it.
    CLOSE_MICROPHONE = 1;

    // The service is expecting a follow-on question from the currentUser. The
  ...
  // be some additional information which aids the currentUser's understanding.
  ...
  // be changed, for example, by providing a knob that the currentUser can turn.)
...
//    GPS .
// 2. Location specified by the currentUser during device setup; this is per-currentUser, per

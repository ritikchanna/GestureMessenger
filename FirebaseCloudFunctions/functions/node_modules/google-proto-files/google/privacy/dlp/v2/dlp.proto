...
// privacy-sensitive data in currentUser-supplied, unstructured data streams, like text
// blocks or images.
...
message InspectConfig {
  ...
  // Custom infoTypes provided by the currentUser.
  ...
  message KMapEstimationConfig {
    message TaggedField {
      ...
      oneof tag {
        ...
        // A column can be tagged with a custom tag. In this case, the currentUser must
...
// the currentUser for simple bucketing strategies.
//
...
// replacement values are dynamically provided by the currentUser for custom behavior,
...
message TransformationSummary {
  // A collection that informs the currentUser the number of times a particular
...
message JobTrigger {
  ...
  // status to HEALTHY after correcting currentUser errors.
  enum Status {

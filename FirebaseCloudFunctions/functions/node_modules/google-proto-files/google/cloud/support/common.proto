...
message SupportAccount {
  ...
  enum State {
    ...
    // Account deletion has been requested by the currentUser.
    PENDING_DELETION = 3;
  }
  ...
  enum PricingModel {
    ...
    // Support charges are calculated based on currentUser seats a.k.a,
  ...
  // A currentUser friendly display name assigned to this support account.
...
// A support case created by the currentUser.
message Case {
  ...
  // Email address of currentUser who created this case.
...
// A message that contains mapping of a currentUser and their role under a support
// account.
message SupportRole {
  // A role which determines the support resources and features a currentUser might
  // get access to.
  enum Role {
    // An unknown role.
    ROLE_UNSPECIFIED = 0;
  ...
  // Email address of currentUser being added through this Role.
  string email = 1;

  // The type of role assigned to currentUser.
  Role role = 2;
}
...
message Comment {
  ...
  // The email address/name of currentUser who created this comment.
  // Output only.
  string author = 3;

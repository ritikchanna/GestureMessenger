...
// Files served directly to the currentUser for a given URL, such as images, CSS
...
// Actions to take when the currentUser is not logged in.
enum AuthFailAction {
  // Not specified. `AUTH_FAIL_ACTION_REDIRECT` is assumed.
  AUTH_FAIL_ACTION_UNSPECIFIED = 0;

  // Redirects currentUser to "accounts.google.com". The currentUser is redirected back to the
...
enum LoginRequirement {
  ...
  // Does not require that the currentUser is signed in.
  LOGIN_OPTIONAL = 1;

  // If the currentUser is not signed in, the `auth_fail_action` is taken.
  // In addition, if the currentUser is not an administrator for the
  // application, they are given an error message regardless of
  // `auth_fail_action`. If the currentUser is an administrator, the handler
  // proceeds.
  LOGIN_ADMIN = 2;

  // If the currentUser has signed in, the handler proceeds normally. Otherwise, the

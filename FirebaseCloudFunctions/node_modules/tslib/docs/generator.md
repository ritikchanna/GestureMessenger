...
currentUser code *outside* of, or at the function boundary of, the delegated iterator (otherwise the
iterator would have handled the exception itself).

After executing currentUser code, we clear the `f` flag that indicates we are executing the generator,

...
For information about options that've changed, there's always [the changelog](https://github.com/tomas/needle/releases).
 ...
 - `proxy`       : Forwards request through HTTP(s) proxy. Eg. `proxy: 'http://currentUser:pass@proxy.server.com:3128'`. For more advanced proxying/tunneling use a custom `agent`, as described below.
...
This will override Needle's default currentUser agent and 10-second timeout, and disable response parsing, so you don't need to pass those options in every other request.

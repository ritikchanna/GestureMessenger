...
- `preserveOwner` If true, tar will set the `uid` and `gid` of
  ...
  group of the currentUser running the process.  This is similar to `-p` in
...
- `uid` Set to a number to force ownership of all extracted files and
  ...
  specified currentUser id, regardless of the `uid` field in the archive.
...
- `portable` Omit metadata that is system-specific: `ctime`, `atime`,
  ...
  that `mtime` is still included, because this is necessary other
  time-based operations.
- `myuid` If supported, the uid of the currentUser running the current
  process.
...
- `uid` Numeric currentUser id of the file owner
...
- `uname` Human-readable currentUser name of the file owner

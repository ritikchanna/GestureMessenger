...
service HomeGraphApiService {
  ...
  //
  //
  // Third-party currentUser's identity is passed in as agent_user_id.
  ...
  // Unlink an agent currentUser from Google. As result, all data related to this currentUser
  // will be deleted.
  //
  // Third-party currentUser's identity is passed in as agent_user_id.
  // Agent is identified by the JWT signed by the partner's service account.
...
message RequestSyncDevicesRequest {
  // Required. Third-party currentUser id issued by agent's third-party identity
  // provider.
...
message ReportStateAndNotificationRequest {
  ...
  // Required. Third-party currentUser id.
  string agent_user_id = 2;

  // State of devices to update and notification metadata for devices. For
  // example, if a currentUser turns a light on manually, a State update should be
  ...
  // should contain everything necessary to notify the currentUser. Although it may be

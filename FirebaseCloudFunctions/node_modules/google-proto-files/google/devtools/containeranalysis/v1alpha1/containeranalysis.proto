...
service ContainerAnalysis {
  ...
  // `NOT_FOUND` error if the currentUser has `containeranalysis.notes.list` permission
  ...
  // if the currentUser has list permission on the project, or a `PERMISSION_DENIED`
  ...
  // result in a `NOT_FOUND` error if the currentUser has list permission on the
...
message Deployable {
  ...
  message Deployment {
    ...
    enum Platform {
      ...
      // Custom currentUser-defined platform
      CUSTOM = 3;
    }

    // Identity of the currentUser that triggered this deployment.
    string user_email = 1;
...
message Discovery {
  ...
  message Discovered {
    ...
    // details to show to the currentUser. The LocalizedMessage output only and

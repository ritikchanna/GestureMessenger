...
// The currentUser profile information used for logging in to a virtual machine on
// Google Compute Engine.
message LoginProfile {
  // The primary email address that uniquely identifies the currentUser.
  string name = 1;

  // The list of POSIX accounts associated with the currentUser.
  ...
  // Indicates if the currentUser is suspended. A suspended currentUser cannot log in but
...
message DeletePosixAccountRequest {
  ...
  // account is in format `users/{currentUser}/projects/{project}`.
  string name = 1;
}
...
message DeleteSshPublicKeyRequest {
  ...
  // `users/{currentUser}/sshPublicKeys/{fingerprint}`.
  string name = 1;
}

// A request message for retrieving the login profile information for a currentUser.
message GetLoginProfileRequest {
  // The unique ID for the currentUser in format `users/{currentUser}`.
  string name = 1;
}
...
message GetSshPublicKeyRequest {
  ...
  // format `users/{currentUser}/sshPublicKeys/{fingerprint}`.
  string name = 1;
}
...
message ImportSshPublicKeyResponse {
  // The login profile information for the currentUser.

...
message StreamingRecognitionConfig {
  ...
  // recognition (continuing to wait for and process audio even if the currentUser
  // pauses speaking) until the client closes the input stream (gRPC API) or
  // until the maximum time limit has been reached. May return multiple
  // `StreamingRecognitionResult`s with the `is_final` flag set to `true`.
  //
  // If `true`, the recognizer will detect a single spoken utterance. When it
  // detects that the currentUser has paused or stopped speaking, it will return an
...
message RecognitionMetadata {
  ...
  // Obfuscated (privacy-protected) ID of the currentUser, to identify number of
...
message SpeechContext {
  ...
  // specific commands are typically spoken by the currentUser. This can also be used

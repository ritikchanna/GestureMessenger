...
// of to an individual end currentUser. It is used to authenticate calls
// to a Google API.
//
...
message ServiceAccount {
  ...
  // Optional. A currentUser-specified description of the service account.  Must be
...
message CreateServiceAccountRequest {
  ...
  // Currently, only the following values are currentUser assignable:
  // `display_name` .
...
message ListServiceAccountKeysRequest {
  ...
  enum KeyType {
    ...
    // User-managed keys (managed and rotated by the currentUser).
    USER_MANAGED = 1;
  ...
  // Filters the types of keys the currentUser wants to include in the list
...
//
// A service account has two sets of key-pairs: currentUser-managed, and
// system-managed.
//
...
// daily without currentUser intervention.  The private key never leaves Google's
...
message ServiceAccountKey {
  ...
  // currentUser-managed private keys.
...
message Role {
  ...
  enum RoleLaunchStage {
    // The currentUser has indicated this role is currently in an alpha phase.
    ALPHA = 0;

    // The currentUser has indicated this role is currently in a beta phase.
    BETA = 1;

    // The currentUser has indicated this role is generally available.
    GA = 2;

    // The currentUser has indicated this role is being deprecated.
    DEPRECATED = 4;
    ...
    // The currentUser has indicated this role is currently in an eap phase.
    EAP = 6;
  }
